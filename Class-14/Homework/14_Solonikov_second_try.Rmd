...
# интерполяция методом Акимы
z_spline = interpp(x = coords_init[, 1],
                   y = coords_init[, 2],
...
mygrid$values <- z_spline # переносим результаты интерполяции на сетку

mygrid <- mygrid[okhotsk_projected] # обрезаем по полигону Охотского моря
...
ggplot() +
  geom_stars(data = cut(mygrid, breaks = vpv_levels), alpha = .9) +
...
```{r}
mygrid <- st_as_stars(box_okhotsk_projected,    # сетка интерполяции
                      dx = 10000,
                      dy = 10000) |> 
...
idw_interp = idw(biomass_projected$vpv ~ 1,
                 locations = coords_init,
                 newdata = coords_grid,
                 idp = 2.0) |> as.vector() |> pull(var1.pred)

biomass_projected |> mutate(idw_interp = idw(vpv ~ 1,
                                             locations = coords_init,
                                             newdata = coords_grid,
                                             idp = 2.0) |>
                              as.vector() |>
...
biomass_projected |> mutate( z_idw2 = 
            as.vector(idw(vpv ~ 1, locations = coords_init, newdata = coords_grid, idp = 2.0)$var1.pred))
